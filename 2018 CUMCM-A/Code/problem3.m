...
for d4 = 6 : 64
    ...
    for d2 = 200 : 220
        ...
        %   t_next(103) = t(i, 103)*(1 - 2*h2*dt/(p(3)*c(3)*dx) - 2*a(3)*dt/dx^2) + 2*a(3)*dt*t(i, 102)/dx^2 + 2*h2*dt*37/(p(3)*c(3)*dx);
            ...
            % 第四层
            t_next(2 + n_three) = t(i, 2 + n_three) + h2*dt*(t(i, 1 + n_three) + t(i, 3 + n_three) - 2*t(i, 2 + n_three)) / ((d(4)/dx)*p(4)*c(4)*dx);

            t(i+1, :) = t_next;     % 预先分配内存提速
        end
        if(t(100 * 1501, end - 1) <= 44 && t(100 * 1801, end - 1) <= 47)
            break;
        end
    end
    disp(d(2));
